...
int main(int argc, char** argv) {
  ...
  const std::string data_dir =
      FLAGS_sift_directory.empty() ? project_folder_path + "/sift" : FLAGS_sift_directory;

  // X. Show data path.
  LOG(INFO) << "Load images from     : " << image_dir;
  LOG(INFO) << "Load sift files from : " << data_dir;
  LOG(INFO) << "Save gmm file to     : " << FLAGS_gmm_path;
  ...
  fisher_encoder.SaveGMM(FLAGS_gmm_path);
